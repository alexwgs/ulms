...
4. 前端渐进 TS 化深入（api 模块 role/dictionary/quickMemo/ad/jobinfo/station/menu/operLog/errorLog/userFiles/dashboard/manageNotice/onlineMonitor 共 13 个已转 .ts，store auth.ts、ui.ts、app.ts 已转；其余 api 模块 → 其余 store → router meta 逐步启用类型）
