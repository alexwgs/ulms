<template>
  <div>
    <t-alert v-if="ohtStore.ohtStatusInfo === 'default0'" style="margin-top: 0" title="操作BAR" theme="info"
      :closable="false" show-icon>
      当前连接正常，如需求助请点击
      <t-button v-if="ohtStore.ohtRole.roleType === 0" theme="success" size="small"
        @click="helpTypeDialogVisible = true">我要求助</t-button>
    </t-alert>

    <t-alert v-if="ohtStore.ohtStatusInfo === 'default1'" style="margin-top: 0" title="操作BAR" theme="info"
      :closable="false" show-icon>
      当前连接正常，当前暂无举手订单！
    </t-alert>

    <t-alert v-if="ohtStore.ohtStatusInfo === 'helpWait'" style="margin-top: 0" title="正在发起求助" theme="success"
      :closable="false" show-icon>
      当前求助订单已开始分派,当前等待 [
    ...
    " style="margin-top: 0" title="建立对话" theme="success" :closable="false" show-icon>
    ...
    <t-dialog header="重要提示" v-model:visible="helpTypeDialogVisible" width="60%" placement="center">
      ...
      <template #footer>
        <t-button v-for="item in dictStore.dict.oht_case_type" :key="item.id" size="small"
          :theme="item.code == 1 ? 'primary' : 'danger'" @click="newCase(parseInt(item.code))">{{ item.codeval
          }}</t-button>
        <t-button size="small" @click="helpTypeDialogVisible = false">取 消</t-button>
      </template>
    </t-dialog>

    <t-dialog header="取消求助" v-model:visible="cancelDialogVisible" width="60%">
      <div>
        <t-form :data="cancelForm" :rules="cancelFormRuleForm" ref="cancelFormRef" label-width="100px"
          class="demo-ruleForm">
          <t-form-item label="操作类型" name="caseStatus">
            <t-select size="small" v-model="cancelForm.caseStatus" readonly>
              <t-option label="取消求助" :value="1"></t-option>
            </t-select>
          </t-form-item>
          <t-form-item label="操作原因" name="bcompType">
            <t-select size="small" v-model="cancelForm.bcompType">
              <t-option v-for="item in dictStore.getDictByNames('oht_case_bcompType').filter(
                (item) => item.code >= 20
              )" :key="item.id" :label="item.codeval" :value="parseInt(item.code)"></t-option>
            </t-select>
          </t-form-item>
          <t-form-item label="操作备注" name="bcompMemo">
            <t-textarea size="small" maxlength="250" v-model="cancelForm.bcompMemo" />
          </t-form-item>
        </t-form>
      </div>
      <template #footer="scope">
        <t-button size="small" theme="primary" @click="cancelSubmit">提 交</t-button>
        <t-button size="small" @click="cancelDialogVisible = false">取 消</t-button>
      </template>
    </t-dialog>

    <t-dialog header="案件结案" v-if="ohtStore.currentCase !== null" v-model:visible="bcaseCompDialogVisible" width="60%">
      <t-divider content-position="left">案件信息</t-divider>
      <t-row :gutter="15">
        <t-col :span="6">案件类型：{{
          dictStore.getDictLabel('oht_case_type', ohtStore.currentCase.caseType)
        }}</t-col>
        <t-col :span="6">发起时间：{{ ohtStore.currentCase.dataTime }}</t-col>
        <t-col :span="6" v-if="ohtStore.currentCase.pickUser">接单人：{{ ohtStore.currentCase.pickUser.ploNum }}/{{
          ohtStore.currentCase.pickUser.ploName
        }}</t-col>
        <t-col :span="6">接单人分机：{{ ohtStore.currentCase.pickExtn }}</t-col>
        <t-col :span="6" v-if="ohtStore.currentCase.pickUser">科室组别：{{ ohtStore.currentCase.pickUser.deptName }}/{{
          ohtStore.currentCase.pickUser.groupName
        }}</t-col>
        <t-col :span="6"></t-col>
      </t-row>
      <t-divider content-position="left">结案表单</t-divider>
      <div>
        <t-form :data="bcompForm" :rules="bcompFormRuleForm" ref="bcompFormRef" label-width="100px"
          class="demo-ruleForm">
          <t-form-item label="结案类型" name="bcompType">
            <t-select size="small" v-model="bcompForm.bcompType">
              <t-option v-for="item in dictStore.getDictByNames('oht_case_bcompType').filter(
                (item) => item.code < 20
              )" :key="item.id" :label="item.codeval" :value="parseInt(item.code)"></t-option>
            </t-select>
          </t-form-item>
          <t-form-item label="案件备注" name="bcompMemo">
            <t-textarea size="small" maxlength="250" v-model="bcompForm.bcompMemo" />
          </t-form-item>
        </t-form>
      </div>
      <template #footer="scope">
        <t-button size="small" theme="primary" @click="bcaseCompSubmit">提 交</t-button>
        <t-button size="small" @click="bcaseCompDialogVisible = false">取 消</t-button>
      </template>
    </t-dialog>
    <t-dialog header="案件结案" v-if="ohtStore.currentCase !== null" v-model:visible="pcaseCompDialogVisible" width="60%">
      <t-divider content-position="left">案件信息</t-divider>
      <t-row :gutter="15">
        <t-col :span="6">案件类型：{{
          dictStore.getDictLabel('oht_case_type', ohtStore.currentCase.caseType)
        ...
        <t-col :span="6">发起时间：{{ ohtStore.currentCase.dataTime }}</t-col>
        <t-col :span="6" v-if="ohtStore.currentCase.buildUser">建案人：{{ ohtStore.currentCase.buildUser.ploNum }}/{{
          ohtStore.currentCase.buildUser.ploName
        }}</t-col>
        <t-col :span="6">建案人分机：{{ ohtStore.currentCase.buildExtn }}</t-col>
        <t-col :span="6" v-if="ohtStore.currentCase.buildUser">科室组别：{{ ohtStore.currentCase.buildUser.deptName }}/{{
      ...
      <t-divider content-position="left">结案表单</t-divider>
      <div>
        <t-form :data="pcompForm" :rules="pcompFormRuleForm" ref="pcompFormRef" label-width="100px"
          class="demo-ruleForm">
          <t-form-item label="客户信息" name="custId">
            <t-input size="small" maxlength="20" v-model="pcompForm.custId" placeholder="ID\证件号\手机号"></t-input>
          </t-form-item>
          <t-form-item label="结案类型" name="pcompType">
            <t-select size="small" v-model="pcompForm.pcompType">
              <t-option v-for="item in dictStore.dict.oht_case_pcompType" :key="item.id" :label="item.codeval"
                :value="parseInt(item.code)"></t-option>
            </t-select>
          </t-form-item>
          <t-form-item label="案件备注" name="pcompMemo">
            <t-textarea size="small" maxlength="250" v-model="pcompForm.pcompMemo" />
          </t-form-item>
        </t-form>
      </div>
      <template #footer="scope">
        <t-button size="small" theme="primary" @click="pcaseCompSubmit">提 交</t-button>
        <t-button size="small" @click="pcaseCompDialogVisible = false">取 消</t-button>
      </template>
    </t-dialog>
    <t-dialog header="拒绝求助" v-model:visible="refuseDialogVisible" width="60%">
      <div>
        <t-form :data="refuseForm" :rules="refuseFormRuleForm" ref="refuseFormRef" label-width="100px"
          class="demo-ruleForm">
          <t-form-item label="操作类型" name="taskStatus">
            <t-select size="small" v-model="refuseForm.taskStatus" readonly>
              <t-option label="拒绝求助" :value="2"></t-option>
            </t-select>
          </t-form-item>
          <t-form-item label="拒绝原因" name="memo">
            <t-textarea size="small" maxlength="250" v-model="refuseForm.memo" />
          </t-form-item>
        </t-form>
      </div>
      <template #footer="scope">
        <t-button size="small" theme="primary" @click="refuseSubmit">提 交</t-button>
        <t-button size="small" @click="refuseDialogVisible = false">取 消</t-button>
      </template>
    </t-dialog>
  </div>
</template>
...
const newCase = (caseType) => {
  if (wsStore.connectionStatus !== 'connected') {
    MessagePlugin.error('WebSocket 未连接，无法发起求助')
    return
  }
...
// 接单
const takeOrder = () => {
  if (!ohtStore.waittingCase[0]) return MessagePlugin.error('订单已不存在!')
  wsStore.sendMessage({
    modal: 'oht', type: 'command',
    content: {
      action: 'takeOrder',
